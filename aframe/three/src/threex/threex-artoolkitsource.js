...
ARjs.Source = THREEx.ArToolkitSource = function(parameters,videostream) {
...
ARjs.Source.prototype._initSourceWebcam = function(onReady, onError) {
	...
	onError = onError || function(error) {
		...
		var event = new CustomEvent('camera-error', {
			error: error
		});
	...
	*/
	...
	domElement.srcObject = this.parameters.videoStream;
	
	var event = new CustomEvent('camera-init', {
		stream: this.parameters.videoStream
	});
	window.dispatchEvent(event);
	// to start the video, when it is possible to start it only on userevent. like in android
	document.body.addEventListener('click', function() {
